...
create table if not exists app_status_mate.org_groups (
...
create unique index if not exists idx_og_slug_unique
  on app_status_mate.org_groups (slug) where deleted_at is null;
create index if not exists idx_og_workspace on app_status_mate.org_groups (workspace_id);
create index if not exists idx_og_owner on app_status_mate.org_groups (owner_user_id);

create table if not exists app_status_mate.org_group_workspaces (
  id            uuid primary key default gen_random_uuid(),
  workspace_id  uuid not null references public.workspaces(id) on delete cascade,
...
create index if not exists idx_ogw_workspace on app_status_mate.org_group_workspaces (workspace_id);
create index if not exists idx_ogw_group on app_status_mate.org_group_workspaces (org_group_id);

create table if not exists app_status_mate.org_group_share_codes (
  id                uuid primary key default gen_random_uuid(),
  workspace_id      uuid not null references public.workspaces(id) on delete cascade,
  org_group_id      uuid not null references app_status_mate.org_groups(id) on delete cascade,
...
create unique index if not exists idx_og_sharecode_active
  on app_status_mate.org_group_share_codes (code) where used_at is null;
create index if not exists idx_og_sharecode_workspace
  on app_status_mate.org_group_share_codes (workspace_id);
create index if not exists idx_og_sharecode_group
  on app_status_mate.org_group_share_codes (org_group_id);

-- ---------- GRANTS + RLS enable ----------

grant select, insert, update, delete on app_status_mate.org_groups to authenticated;
grant select, insert, update, delete on app_status_mate.org_group_workspaces to authenticated;
grant select, insert, update, delete on app_status_mate.org_group_share_codes to authenticated;

alter table app_status_mate.org_groups enable row level security;
alter table app_status_mate.org_group_workspaces enable row level security;
alter table app_status_mate.org_group_share_codes enable row level security;

-- ---------- POLICIES (sau khi mọi table đã tồn tại) ----------

-- org_groups: SELECT mở cross-ws (member origin ws HOẶC member ws đã subscribe).
drop policy if exists "workspace_isolation" on app_status_mate.org_groups;
create policy "workspace_isolation" on app_status_mate.org_groups
for select using (
  workspace_id in (select workspace_id from public.workspace_members where user_id = auth.uid())
  or id in (
    select org_group_id from app_status_mate.org_group_workspaces
...
drop policy if exists "workspace_isolation" on app_status_mate.org_group_workspaces;
create policy "workspace_isolation" on app_status_mate.org_group_workspaces
for all using (
  workspace_id in (select workspace_id from public.workspace_members where user_id = auth.uid())
) with check (
  workspace_id in (select workspace_id from public.workspace_members where user_id = auth.uid())
...
drop policy if exists "workspace_isolation" on app_status_mate.org_group_share_codes;
create policy "workspace_isolation" on app_status_mate.org_group_share_codes
for all using (
  workspace_id in (select workspace_id from public.workspace_members where user_id = auth.uid())
) with check (
  workspace_id in (select workspace_id from public.workspace_members where user_id = auth.uid())
...
create or replace function app_status_mate.user_can_see_org_group(p_group_id uuid)
returns boolean
language sql stable security definer set search_path = app_status_mate as $$
  select exists (
    select 1 from app_status_mate.org_groups g
...
$$;
grant execute on function app_status_mate.user_can_see_org_group(uuid) to authenticated;

-- ---------- RPC: create_org_group ----------
create or replace function app_status_mate.create_org_group(
  p_workspace_id uuid,
  p_name text,
  p_description text default null
)
returns app_status_mate.org_groups
language plpgsql security definer set search_path = app_status_mate as $$
declare
  v_user uuid := auth.uid();
  v_slug text;
  v_suffix text;
  v_row app_status_mate.org_groups;
begin
...
grant execute on function app_status_mate.create_org_group(uuid, text, text) to authenticated;

-- ---------- RPC: generate_org_group_share_code ----------
create or replace function app_status_mate.generate_org_group_share_code(
  p_workspace_id uuid,
  p_group_id uuid,
  p_expires_hours int default 24
)
returns app_status_mate.org_group_share_codes
language plpgsql security definer set search_path = app_status_mate as $$
declare
  v_user uuid := auth.uid();
  v_code text;
  v_row app_status_mate.org_group_share_codes;
  v_attempts int := 0;
begin
  if v_user is null then raise exception 'unauthenticated'; end if;
  if not exists (
  ...
  loop
    ...
    if not exists (
      select 1 from app_status_mate.org_group_share_codes
      where code = v_code and used_at is null
    ) then exit; end if;
    if v_attempts > 50 then raise exception 'no_free_code_found'; end if;
  end loop;

  insert into app_status_mate.org_group_share_codes
...
grant execute on function app_status_mate.generate_org_group_share_code(uuid, uuid, int) to authenticated;

-- ---------- RPC: redeem_org_group_share_code ----------
create or replace function app_status_mate.redeem_org_group_share_code(
  p_code text,
  p_target_ws_id uuid
)
returns app_status_mate.org_groups
...
declare
  v_user uuid := auth.uid();
  v_sc app_status_mate.org_group_share_codes;
  v_group app_status_mate.org_groups;
  v_role text;
begin
  if v_user is null then raise exception 'unauthenticated'; end if;
  ...
  update app_status_mate.org_group_share_codes
  set used_at = now(), used_by = v_user, used_workspace_id = p_target_ws_id
  where id = v_sc.id;

  return v_group;
end $$;
grant execute on function app_status_mate.redeem_org_group_share_code(text, uuid) to authenticated;

-- ---------- RPC: unshare_org_group_from_workspace ----------
create or replace function app_status_mate.unshare_org_group_from_workspace(
  p_group_id uuid,
  p_ws_id uuid
)
returns void
...
begin
  ...
  delete from app_status_mate.org_group_workspaces
  where org_group_id = p_group_id and workspace_id = p_ws_id;
end $$;
grant execute on function app_status_mate.unshare_org_group_from_workspace(uuid, uuid) to authenticated;

-- ---------- RPC: update_org_group (owner only) ----------
create or replace function app_status_mate.update_org_group(
  p_group_id uuid,
...
grant execute on function app_status_mate.update_org_group(uuid, text, text) to authenticated;

-- ---------- RPC: delete_org_group_soft (owner only) ----------
create or replace function app_status_mate.delete_org_group_soft(
  p_group_id uuid,
  p_confirm_slug text
)
returns void
...
begin
  ...
  update app_status_mate.org_groups set deleted_at = now() where id = p_group_id;
end $$;
grant execute on function app_status_mate.delete_org_group_soft(uuid, text) to authenticated;
