...
create or replace function app_org_chart.is_org_group_editor(p_group_id uuid)
returns boolean
language sql stable security definer set search_path = app_org_chart as $$
  select exists (
    select 1
    from app_org_chart.org_group_workspaces gw
...
$$;
grant execute on function app_org_chart.is_org_group_editor(uuid) to authenticated;

-- User là member của ANY ws đã subscribe group? (basic visibility)
create or replace function app_org_chart.is_org_group_member(p_group_id uuid)
returns boolean
language sql stable security definer set search_path = app_org_chart as $$
  select exists (
    select 1
...
$$;
grant execute on function app_org_chart.is_org_group_member(uuid) to authenticated;

-- Guard nội bộ.
create or replace function app_org_chart._require_group_editor(p_group_id uuid)
returns void language plpgsql security definer set search_path = app_org_chart as $$
begin
  if not app_org_chart.is_org_group_editor(p_group_id) then
...
create or replace function app_org_chart._group_origin_ws(p_group_id uuid)
returns uuid
language sql stable security definer set search_path = app_org_chart as $$
  select workspace_id from app_org_chart.org_groups where id = p_group_id;
$$;

-- ---------- DROP RPCs cũ ----------
-- Cần drop trước khi create or replace vì signature đổi (param khác).

drop function if exists app_org_chart.seed_default_positions(uuid);
drop function if exists app_org_chart.create_squad(uuid, text, text, uuid, text);
drop function if exists app_org_chart.update_squad(uuid, text, text, uuid, text);
drop function if exists app_org_chart.assign_squad_lead(uuid, uuid, text);
drop function if exists app_org_chart.create_position(uuid, text);
drop function if exists app_org_chart.delete_position(uuid);
drop function if exists app_org_chart.admin_set_member(uuid, uuid, text, int, text);
drop function if exists app_org_chart.remove_member(uuid, uuid);
drop function if exists app_org_chart.set_my_allocation(uuid, int, text);
drop function if exists app_org_chart.request_membership(uuid, text, text, int, text);
drop function if exists app_org_chart.decide_membership(uuid, boolean);
drop function if exists app_org_chart.cancel_my_request(uuid);
drop function if exists app_org_chart._require_ws_admin(uuid);

-- ---------- seed_default_positions ----------
create or replace function app_org_chart.seed_default_positions(p_group_id uuid)
returns void language plpgsql security definer set search_path = app_org_chart as $$
declare
  ...
  v_ws uuid;
begin
  perform app_org_chart._require_group_editor(p_group_id);
  v_ws := app_org_chart._group_origin_ws(p_group_id);
  if exists (select 1 from app_org_chart.positions where org_group_id = p_group_id) then
    return;
  end if;
  foreach n in array v_names loop
    insert into app_org_chart.positions
...
begin
  ...
  if p_parent is not null and not exists (
    select 1 from app_org_chart.squads where id = p_parent and org_group_id = p_group_id
  ) then raise exception 'parent squad không thuộc org_group này'; end if;
  insert into app_org_chart.squads
...
begin
  ...
  v_is_editor := app_org_chart.is_org_group_editor(s.org_group_id);
  ...
  if v_is_editor then
    ...
    if p_parent is not null and not exists (
      select 1 from app_org_chart.squads where id = p_parent and org_group_id = s.org_group_id
...
begin
  ...
  if s is null then raise exception 'squad không tồn tại'; end if;
  perform app_org_chart._require_group_editor(s.org_group_id);
  v_ws := app_org_chart._group_origin_ws(s.org_group_id);
...
-- ---------- create_position ----------
create or replace function app_org_chart.create_position(p_group_id uuid, p_name text)
returns uuid
language plpgsql security definer set search_path = app_org_chart as $$
declare v_id uuid; v_ws uuid;
begin
  ...
  values (v_ws, p_group_id, btrim(p_name), auth.uid(),
    coalesce((select max(sort_order) + 1 from app_org_chart.positions where org_group_id = p_group_id), 0))
  returning id into v_id;
  return v_id;
end $$;
...
begin
  ...
  perform app_org_chart._require_group_editor(p.org_group_id);
...
begin
  ...
  if not (app_org_chart.is_org_group_editor(s.org_group_id) or s.lead_user_id = auth.uid()) then
...
begin
  ...
  if not app_org_chart.is_org_group_member(s.org_group_id) then
