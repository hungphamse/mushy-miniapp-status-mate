create table if not exists app_status_mate.meeting_rooms (id uuid primary key default gen_random_uuid(),workspace_id uuid not null references public.workspaces(id) on delete cascade,org_group_id uuid not null references app_status_mate.org_groups(id) on delete cascade,title text not null check (char_length(title) between 1 and 120),host_user_id uuid not null references auth.users(id),status text not null default 'scheduled'
check (status in ('scheduled','active','ended')),planned_end_at timestamptz,started_at timestamptz,ended_at timestamptz,ended_by_user_id uuid references auth.users(id),created_by uuid not null references auth.users(id),created_at timestamptz not null default now(),updated_at timestamptz not null default now());
...
create table if not exists app_status_mate.meeting_participants (id uuid primary key default gen_random_uuid(),workspace_id uuid not null references public.workspaces(id) on delete cascade,org_group_id uuid not null references app_status_mate.org_groups(id) on delete cascade,room_id uuid not null references app_status_mate.meeting_rooms(id) on delete cascade,user_id uuid not null references auth.users(id),role text not null default 'member'
check (role in ('host','co_host','member')),joined_at timestamptz not null default now(),left_at timestamptz,meeting_status_applied boolean not null default false,created_by uuid not null references auth.users(id),created_at timestamptz not null default now(),updated_at timestamptz not null default now(),unique (room_id,user_id));
...
or app_status_mate.user_can_see_org_group(org_group_id)) with check (workspace_id in (select workspace_id from public.workspace_members where user_id=auth.uid())
...
create or replace function app_status_mate.user_can_see_org_group(p_group_id uuid) returns boolean language sql stable security definer set search_path=app_status_mate as $$ select exists (select 1 from app_status_mate.org_groups g where g.id=p_group_id and g.deleted_at is null and (g.workspace_id in (select workspace_id from public.workspace_members where user_id=auth.uid()) or exists (select 1 from app_status_mate.org_group_workspaces gw join public.workspace_members wm on wm.workspace_id=gw.workspace_id where gw.org_group_id=g.id and wm.user_id=auth.uid()))); $$;
create or replace function app_status_mate.is_org_group_member(p_group_id uuid) returns boolean language sql stable security definer set search_path=app_status_mate as $$ select exists (select 1 from app_status_mate.org_group_workspaces gw join public.workspace_members wm on wm.workspace_id=gw.workspace_id where gw.org_group_id=p_group_id and wm.user_id=auth.uid()); $$;
create or replace function app_status_mate._group_origin_ws(p_group_id uuid) returns uuid language sql stable security definer set search_path=app_status_mate as $$ select workspace_id from app_status_mate.org_groups where id=p_group_id; $$;
...
(workspace_id,org_group_id,room_id,user_id,action,from_status,to_status,triggered_by_user_id,details,created_by)
values
...
as $$
select case
when exists (select 1 from app_status_mate.meeting_rooms r
where r.id=p_room_id and r.host_user_id=p_user_id) then 'host'
when exists (select 1 from app_status_mate.meeting_participants mp
where mp.room_id=p_room_id
and mp.user_id=p_user_id
and mp.role='co_host'
and mp.left_at is null) then 'co_host'
else 'member'
end;
$$;
...
and (g.workspace_id in (select workspace_id
...
else jsonb_build_object('status',ms.status,'message',ms.message,'status_until',ms.status_until,'reason',ms.reason,'source',ms.source,'set_by_user_id',ms.set_by_user_id,'custom_reason_text',ms.custom_reason_text,'meeting_room_id',ms.meeting_room_id,'previous_status',ms.previous_status,'updated_at',ms.updated_at)
end
...
(workspace_id,org_group_id,user_id,status,message,status_until,reason,source,set_by_user_id,custom_reason_text,previous_status,meeting_room_id,created_by)
values
((select workspace_id from app_status_mate.org_groups where id=p_group_id),p_group_id,p_user_id,coalesce(p_snapshot->>'status','available'),nullif(p_snapshot->>'message',''),nullif(p_snapshot->>'status_until','')::timestamptz,nullif(p_snapshot->>'reason',''),coalesce(nullif(p_snapshot->>'source',''),'self'),nullif(p_snapshot->>'set_by_user_id','')::uuid,nullif(p_snapshot->>'custom_reason_text',''),p_snapshot->'previous_status',nullif(p_snapshot->>'meeting_room_id','')::uuid,coalesce(auth.uid(),p_user_id))
...
(workspace_id,org_group_id,user_id,status,message,status_until,reason,source,set_by_user_id,custom_reason_text,previous_status,meeting_room_id,created_by)
values
(v_room.workspace_id,v_room.org_group_id,p_user_id,p_status,nullif(btrim(coalesce(p_message,'')),''),v_final_until,v_reason,p_source,auth.uid(),null,v_snapshot,p_room_id,auth.uid())
on conflict (org_group_id,user_id)
do update set
status=excluded.status,message=excluded.message,status_until=excluded.status_until,reason=excluded.reason,source=excluded.source,set_by_user_id=excluded.set_by_user_id,custom_reason_text=excluded.custom_reason_text,previous_status=coalesce(app_status_mate.member_statuses.previous_status,excluded.previous_status),meeting_room_id=excluded.meeting_room_id,updated_at=now();
insert into app_status_mate.meeting_participants
(workspace_id,org_group_id,room_id,user_id,role,joined_at,left_at,meeting_status_applied,created_by)
values
(v_room.workspace_id,v_room.org_group_id,p_room_id,p_user_id,app_status_mate._meeting_participant_role(p_room_id,p_user_id),coalesce((select joined_at from app_status_mate.meeting_participants
where room_id=p_room_id and user_id=p_user_id),now()),null,true,auth.uid())
...
role='host',left_at=null,meeting_status_applied=false,updated_at=now();
...
on conflict (room_id,user_id) do update set
role='co_host',left_at=null,meeting_status_applied=false,updated_at=now();
end loop;
perform app_status_mate._append_status_event(v_ws,p_group_id,v_room.id,v_user,'meeting_create',null,'scheduled',v_user,jsonb_build_object('title',btrim(p_title),'planned_end_at',p_planned_end_at));
return v_room;
end $$;
...
(workspace_id,org_group_id,room_id,user_id,role,created_by)
values
(v_room.workspace_id,v_room.org_group_id,p_room_id,v_user,app_status_mate._meeting_participant_role(p_room_id,v_user),auth.uid())
on conflict (room_id,user_id) do update set
role=excluded.role,left_at=null,meeting_status_applied=false,updated_at=now();
end loop;
end $$;
...
and mp.left_at is null
loop
perform app_status_mate._apply_meeting_status(p_room_id,v_user,'in_meeting',null,p_until,'meeting_sync','meeting_apply');
end loop;
else
foreach v_user in array p_user_ids loop
if v_user is null then
...
update app_status_mate.meeting_rooms
set status='ended',ended_at=now(),ended_by_user_id=auth.uid()
where id=p_room_id;
perform app_status_mate._append_status_event(v_room.workspace_id,v_room.org_group_id,p_room_id,v_room.host_user_id,'meeting_end',null,'ended',auth.uid(),jsonb_build_object('restore',p_restore));
if p_restore then
...
(workspace_id,org_group_id,user_id,status,message,status_until,reason,source,set_by_user_id,custom_reason_text,previous_status,meeting_room_id,created_by)
values
(v_ws,p_group_id,auth.uid(),p_status,nullif(btrim(coalesce(p_message,'')),''),v_until,v_reason,'self',auth.uid(),v_custom,null,null,auth.uid())
on conflict (org_group_id,user_id)
do update set
status=excluded.status,message=excluded.message,status_until=excluded.status_until,reason=excluded.reason,source='self',set_by_user_id=auth.uid(),custom_reason_text=excluded.custom_reason_text,previous_status=null,meeting_room_id=null,updated_at=now();
perform app_status_mate._append_status_event(v_ws,p_group_id,null,auth.uid(),v_action,coalesce(v_prev.status,'available'),p_status,auth.uid(),jsonb_build_object('reason',v_reason,'until',v_until));
end $$;
...
perform app_status_mate._append_status_event(v_ws,p_group_id,null,auth.uid(),case when v_prev.source in ('host','meeting_sync') then 'override_status' else 'auto_reset' end,v_prev.status,'available',auth.uid(),jsonb_build_object('source',v_prev.source));
end $$;
