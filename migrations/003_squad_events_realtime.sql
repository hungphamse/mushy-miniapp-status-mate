...
create index if not exists idx_se_ws_created
  on app_org_chart.squad_events (workspace_id, created_at desc);
create index if not exists idx_se_squad
  on app_org_chart.squad_events (workspace_id, squad_id);

grant select on app_org_chart.squad_events to authenticated;
alter table app_org_chart.squad_events enable row level security;

drop policy if exists "workspace_isolation" on app_org_chart.squad_events;
create policy "workspace_isolation" on app_org_chart.squad_events
for all using (
...
-- Trigger: squads
-- ---------------------------------------------------------------------
create or replace function app_org_chart._trg_squads()
returns trigger language plpgsql security definer
set search_path = app_org_chart, public as $$
begin
  if TG_OP = 'INSERT' then
    perform app_org_chart._ev(NEW.workspace_id, NEW.id, null,
  ...
  if NEW.lead_user_id is distinct from OLD.lead_user_id then
    perform app_org_chart._ev(NEW.workspace_id, NEW.id, NEW.lead_user_id,
      'lead_changed', jsonb_build_object('lead_user_id', NEW.lead_user_id));
  end if;
  if NEW.intro is distinct from OLD.intro then
    perform app_org_chart._ev(NEW.workspace_id, NEW.id, null, 'intro_updated', '{}'::jsonb);
  end if;
...
drop trigger if exists trg_ev_squads on app_org_chart.squads;
create trigger trg_ev_squads
  after insert or update on app_org_chart.squads
  for each row execute function app_org_chart._trg_squads();

-- ---------------------------------------------------------------------
-- Trigger: squad_members
-- ---------------------------------------------------------------------
create or replace function app_org_chart._trg_members()
returns trigger language plpgsql security definer
set search_path = app_org_chart, public as $$
begin
  if TG_OP = 'INSERT' then
    perform app_org_chart._ev(NEW.workspace_id, NEW.squad_id, NEW.user_id,
...
drop trigger if exists trg_ev_members on app_org_chart.squad_members;
create trigger trg_ev_members
  after insert or update on app_org_chart.squad_members
  for each row execute function app_org_chart._trg_members();

-- ---------------------------------------------------------------------
-- Trigger: membership_requests (mig 002)
-- ---------------------------------------------------------------------
create or replace function app_org_chart._trg_requests()
returns trigger language plpgsql security definer
set search_path = app_org_chart, public as $$
begin
  if TG_OP = 'INSERT' then
...
drop trigger if exists trg_ev_requests on app_org_chart.membership_requests;
create trigger trg_ev_requests
  after insert or update on app_org_chart.membership_requests
  for each row execute function app_org_chart._trg_requests();
