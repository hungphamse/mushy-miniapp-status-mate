...
--
-- Slug "org-chart" → schema "app_status_mate" (dash → underscore).
...
-- ---------------------------------------------------------------------
create or replace function app_status_mate.set_updated_at() returns trigger as $$
begin new.updated_at = now(); return new; end;
$$ language plpgsql;

drop trigger if exists trg_squads_updated_at on app_status_mate.squads;
create trigger trg_squads_updated_at before update on app_status_mate.squads
  for each row execute function app_status_mate.set_updated_at();

drop trigger if exists trg_sm_updated_at on app_status_mate.squad_members;
create trigger trg_sm_updated_at before update on app_status_mate.squad_members
  for each row execute function app_status_mate.set_updated_at();

-- ---------------------------------------------------------------------
-- 5. RLS — SELECT mở cho member workspace; WRITE chặn (chỉ RPC definer)
-- ---------------------------------------------------------------------
grant select on app_status_mate.squads        to authenticated;
grant select on app_status_mate.positions     to authenticated;
grant select on app_status_mate.squad_members to authenticated;

alter table app_status_mate.squads        enable row level security;
alter table app_status_mate.positions     enable row level security;
alter table app_status_mate.squad_members enable row level security;
...
--    search_path khoá vào app_status_mate, public (an toàn injection).
-- ---------------------------------------------------------------------

-- 6.1 Guard nội bộ — workspace admin?
create or replace function app_status_mate._require_ws_admin(p_ws uuid)
returns void language plpgsql security definer set search_path = app_status_mate, public as $$
begin
...
create or replace function app_status_mate.seed_default_positions(p_ws uuid)
returns void language plpgsql security definer set search_path = app_status_mate, public as $$
declare v_names text[] := array['Product','Tech','Design','QC','Ops']; n text; i int := 0;
begin
  perform app_status_mate._require_ws_admin(p_ws);
  if exists (select 1 from app_status_mate.positions where workspace_id = p_ws) then
    return;
  end if;
  foreach n in array v_names loop
    insert into app_status_mate.positions (workspace_id, name, is_system, sort_order, created_by)
...
) returns uuid language plpgsql security definer set search_path = app_status_mate, public as $$
declare v_id uuid;
begin
  perform app_status_mate._require_ws_admin(p_ws);
  if p_parent is not null and not exists (
    select 1 from app_status_mate.squads where id = p_parent and workspace_id = p_ws
  ) then raise exception 'parent squad không thuộc workspace này'; end if;
  insert into app_status_mate.squads (workspace_id, parent_id, slug, name, intro, created_by)
...
begin
  ...
  if s is null then raise exception 'squad không tồn tại'; end if;
  perform app_status_mate._require_ws_admin(s.workspace_id);
...
create or replace function app_status_mate.create_position(p_ws uuid, p_name text)
returns uuid language plpgsql security definer set search_path = app_status_mate, public as $$
declare v_id uuid;
begin
  perform app_status_mate._require_ws_admin(p_ws);
  insert into app_status_mate.positions (workspace_id, name, created_by,
    sort_order)
  values (p_ws, btrim(p_name), auth.uid(),
    coalesce((select max(sort_order)+1 from app_status_mate.positions where workspace_id = p_ws), 0))
  returning id into v_id;
  return v_id;
end $$;
...
begin
  ...
  perform app_status_mate._require_ws_admin(p.workspace_id);
