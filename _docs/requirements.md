...
> Mini-app slug `org-chart` → schema `app_org_chart`. Repo: `mushy-miniapp-orgchart`.
...
- Việc chia 2 khối:
  ...
  - **KHỐI B — mini-app `org-chart`** (repo này): squads, positions, membership, allocation, org chart viz.
...
| A6 | Visibility cross-user trong workspace: helper workspace-mate (superapp mig 004) phải expose thêm `full_name`/`job_title`/`work_phone` để org-chart đọc người khác. |

---

## 2. KHỐI B — Mini-app `org-chart` (repo này)
